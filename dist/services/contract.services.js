...
class contractService {
    ...
    static getCancelledContractsInTimeRange(startTime, endTime) {
        return __awaiter(this, void 0, void 0, function* () {
            // Convert seconds to Date objects
            const startDate = new Date(startTime * 1000);
            const endDate = new Date(endTime * 1000);
            // Query cancelled contracts within the time range
            const cancelledContracts = yield contractRepo
                .createQueryBuilder("contract")
    ...
    static getContractsInTimeRange(startTime, endTime, status) {
        return __awaiter(this, void 0, void 0, function* () {
            // Validate status
            const validStatuses = [
                "draft",
            ...
            const contracts = yield contractRepo
                .createQueryBuilder("contract")
    ...
    static getCustomerContractReport() {
        return __awaiter(this, void 0, void 0, function* () {
            const report = yield contractRepo
                .createQueryBuilder("contract")
            ...
            const formattedReport = report.map((customer) => ({
                ...
                statistics: {
                    ...
                    draft: {
                        ...
                        percentage: ((parseInt(customer.draftContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                    pending: {
                        count: parseInt(customer.pendingContracts),
                        percentage: ((parseInt(customer.pendingContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                    readyToSign: {
                        count: parseInt(customer.readyToSignContracts),
                        percentage: ((parseInt(customer.readyToSignContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                    cancelled: {
                        count: parseInt(customer.cancelledContracts),
                        percentage: ((parseInt(customer.cancelledContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                    completed: {
                        count: parseInt(customer.completedContracts),
                        percentage: ((parseInt(customer.completedContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                    rejected: {
                        count: parseInt(customer.rejectedContracts),
                        percentage: ((parseInt(customer.rejectedContracts) /
                            parseInt(customer.totalContracts)) *
                            100).toFixed(1),
                    },
                },
            ...
            return {
                ...
                summary: {
                    ...
                    draft: {
                        ...
                        percentage: ((totals.totalDraft / totals.totalContracts) *
                            100).toFixed(1),
                    },
                    pending: {
                        count: totals.totalPending,
                        percentage: ((totals.totalPending / totals.totalContracts) *
                            100).toFixed(1),
                    },
                    readyToSign: {
                        count: totals.totalReadyToSign,
                        percentage: ((totals.totalReadyToSign / totals.totalContracts) *
                            100).toFixed(1),
                    },
                    cancelled: {
                        count: totals.totalCancelled,
                        percentage: ((totals.totalCancelled / totals.totalContracts) *
                            100).toFixed(1),
                    },
                    completed: {
                        count: totals.totalCompleted,
                        percentage: ((totals.totalCompleted / totals.totalContracts) *
                            100).toFixed(1),
                    },
                    rejected: {
                        count: totals.totalRejected,
                        percentage: ((totals.totalRejected / totals.totalContracts) *
                            100).toFixed(1),
                    },
                },
            };
    ...
    static getAdvancedStatistics(_a) {
        return __awaiter(this, arguments, void 0, function* ({ startTime, endTime, status, createdById, customerId, }) {
            ...
            const statusStats = yield queryBuilder
                ...
                .getRawMany();
            // Thống kê theo tháng
            const monthlyStats = yield queryBuilder
            ...
            if (createdById && customerId) {
                creatorCustomerStats = yield queryBuilder
                    .select("contract.status", "status")
                    .addSelect("COUNT(contract.id)", "count")
            ...
            if (createdById && customerId) {
                result["creatorCustomerRelationship"] = {
                    total: creatorCustomerStats.reduce((sum, item) => sum + parseInt(item.count), 0),
